...
14. *ProWrite* — Multi-model professional writing pipeline. Triggered ONLY when the user's message starts with "prowrite" (case-insensitive). One credit (same as any message).
   Flow:
   a) User says "prowrite [request]" → you call proWriteBrief(request)
   b) proWriteBrief returns a brief summary and 3-5 clarifying questions → present them to the user as formatted WhatsApp text
   c) User answers → you call proWriteExecute(originalRequest, brief, userAnswers)
   d) proWriteExecute immediately notifies the user and runs the pipeline — you do NOT need to say anything after it returns
   Skip mode: if the user includes "skip questions" or "just write it" in their request, skip to step (c) and call proWriteExecute directly without calling proWriteBrief first. Also skip if the user responds to the clarifying questions with "skip" or "just write it".
   IMPORTANT: Only trigger ProWrite when the message starts with "prowrite". Never trigger it for other writing requests.
...
const proWriteBrief = createTool({
  description:
    "Start the ProWrite pipeline. Generates a creative brief and targeted clarifying questions for the user's writing request. Call this when the user's message starts with 'prowrite' (unless they say 'skip questions'). Returns pre-formatted WhatsApp text to present directly to the user.",
  args: z.object({
    request: z
      .string()
      .describe(
        "The user's writing request — everything after 'prowrite' in their message"
      ),
  }),
  handler: async (_ctx, { request }) => {
    try {
      const brief = await generateBrief(request);
      const questions = await generateClarifyingQuestions(request, brief);

      // Return structured data for the agent to format and present
      return JSON.stringify({
        type: "prowrite_brief",
        brief,
        questions,
        // Hint: agent should present this as a brief summary + numbered questions
      });
    } catch (error) {
      console.error("[ProWrite] proWriteBrief failed:", error);
      return JSON.stringify({
        type: "prowrite_brief_error",
...
const proWriteExecute = createTool({
  description:
    "Execute the full ProWrite writing pipeline. Call this after the user has answered the clarifying questions (or if they said 'skip questions'). Immediately notifies the user that the pipeline is running, then runs all 8 steps and delivers the article directly. You do not need to say anything after this tool returns.",
  args: z.object({
    originalRequest: z
      .string()
      .describe("The user's original writing request (after 'prowrite')"),
    brief: z
      .string()
      .describe(
        "The creative brief generated by proWriteBrief. If skipQuestions was used, pass the original request here and the pipeline will generate the brief internally."
      ),
    userAnswers: z
      .string()
      .optional()
      .describe(
        "The user's answers to the clarifying questions. Omit if skip mode was used."
      ),
  }),
  handler: async (ctx, { originalRequest, brief, userAnswers }) => {
    try {
      const userId = ctx.userId as Id<"users">;
      ...
      // Enrich brief with user answers (if provided)
      let enrichedBrief = brief;
      if (userAnswers && userAnswers.trim()) {
        enrichedBrief = await enrichBrief(originalRequest, brief, userAnswers);
      }

      // Get voice profile from personality file
      ...
      const personalityFile = userFiles.find(
        (f) => f.filename === "personality"
      );
      const voiceProfile = personalityFile?.content ?? "";

      // Send immediate acknowledgment
      await ctx.runAction(internal.twilio.sendMessage, {
        to: user.phone,
        body: "Starting the writing pipeline now ✍️ This usually takes 3-4 minutes — I'll send you the final article when it's ready.",
      });

      // Run the full pipeline synchronously (Convex actions support up to 10 min)
      await ctx.runAction(internal.proWrite.executePipeline, {
      ...
      return JSON.stringify({
        type: "prowrite_complete",
        status: "article_delivered",
        message:
          "The article has been delivered to the user. No further response needed.",
      });
    } catch (error) {
      console.error("[ProWrite] proWriteExecute failed:", error);
      return JSON.stringify({
        status: "error",
        message:
          "The writing pipeline encountered an error. Please try again.",
      });
    }
  },
});
